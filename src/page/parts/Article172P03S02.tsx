import React from "react";

export function Article172P03S02() {
  return (
      <div className="flex pt-[80px] pr-[120px] pb-[80px] pl-[120px] flex-col gap-[40px] items-start self-stretch shrink-0 flex-nowrap bg-[#fff] relative z-[228]">
        <div className="flex flex-col gap-[12px] items-center self-stretch shrink-0 flex-nowrap relative z-[229]">
          <span className="flex w-[457px] h-[44px] justify-center items-start shrink-0 basis-auto font-['Vazirmatn'] text-[28px] font-extrabold leading-[43.75px] text-[#17324d] relative text-center whitespace-nowrap z-[230]">
            چه اطلاعاتی ممکن است درخواست شود؟
          </span>
          <span className="h-[23px] self-stretch shrink-0 basis-auto font-['Vazirmatn'] text-[15px] font-normal leading-[23px] text-[#60758a] relative text-center whitespace-nowrap z-[231]">
            فهرست دقیق مدارک به نوع مشارکت و ضوابط جاری وابسته است. موارد زیر
            فقط نمونه عمومی هستند.
          </span>
        </div>
        <div className="flex pt-[32px] pr-[32px] pb-[32px] pl-[32px] flex-col gap-[24px] items-start self-stretch shrink-0 flex-nowrap bg-[#fcfbf8] rounded-[24px] border-solid border border-[#e4ebf1] relative shadow-[0_4px_16px_0_rgba(22,45,71,0.08)] z-[232]">
          <div className="flex justify-between items-center self-stretch shrink-0 flex-nowrap relative z-[233]">
            <div className="flex w-[207px] pt-[11px] pr-[20px] pb-[11px] pl-[20px] gap-[8px] justify-center items-center shrink-0 flex-nowrap bg-[#fff] rounded-[12px] border-solid border border-[#e4ebf1] relative z-[234]">
              <span className="flex w-[167px] h-[22px] justify-center items-start shrink-0 basis-auto font-['Vazirmatn'] text-[14px] font-medium leading-[22px] text-[#2094e3] relative text-center whitespace-nowrap z-[235]">
                مشاهده درخواست‌های شرکت
              </span>
            </div>
            <span className="flex w-[271px] h-[28px] justify-end items-start shrink-0 basis-auto font-['Vazirmatn'] text-[18px] font-bold leading-[28px] text-[#17324d] relative text-right whitespace-nowrap z-[236]">
              نمونه اسناد و مستندات بررسی عمومی
            </span>
          </div>
          <div className="h-px self-stretch shrink-0 bg-[url('/assets/codia/iyWC1H6Onp.png')] bg-cover bg-no-repeat relative z-[237]" />
          <div className="self-stretch shrink-0 relative z-[238]">
            <div className="flex w-[1136px] h-[22px] justify-between items-center relative z-[247]">
              <div className="flex gap-[12px] justify-end items-center grow shrink-0 basis-0 flex-nowrap relative z-[239]">
                <span className="h-[22px] grow shrink-0 basis-auto font-['Vazirmatn'] text-[14px] font-normal leading-[21.875px] text-[#17324d] relative text-right whitespace-nowrap z-[240]">
                  شناسه ملی شرکت و شماره ثبت اسناد
                </span>
                <div className="flex w-[18px] h-[18px] flex-col justify-center items-center shrink-0 flex-nowrap relative overflow-hidden z-[241]">
                  <div className="w-[18px] h-[18px] shrink-0 bg-[url('/assets/codia/8D9YjPvTnV.png')] bg-cover bg-no-repeat relative overflow-hidden z-[242]" />
                </div>
              </div>
              <div className="flex gap-[12px] justify-end items-center grow shrink-0 basis-0 flex-nowrap relative z-[243]">
                <span className="h-[22px] grow shrink-0 basis-auto font-['Vazirmatn'] text-[14px] font-normal leading-[21.875px] text-[#17324d] relative text-right whitespace-nowrap z-[244]">
                  کد اقتصادی فعال شخصیت حقوقی
                </span>
                <div className="flex w-[18px] h-[18px] flex-col justify-center items-center shrink-0 flex-nowrap relative overflow-hidden z-[245]">
                  <div className="w-[18px] h-[18px] shrink-0 bg-[url('/assets/codia/9BDeWGDHzm.png')] bg-cover bg-no-repeat relative overflow-hidden z-[246]" />
                </div>
              </div>
              <div className="flex gap-[12px] justify-end items-center grow shrink-0 basis-0 flex-nowrap relative z-[247]">
                <span className="h-[22px] grow shrink-0 basis-auto font-['Vazirmatn'] text-[14px] font-normal leading-[21.875px] text-[#17324d] relative text-right whitespace-nowrap z-[248]">
                  روزنامه رسمی آخرین تغییرات و صاحبان امضا
                </span>
                <div className="flex w-[18px] h-[18px] flex-col justify-center items-center shrink-0 flex-nowrap relative overflow-hidden z-[249]">
                  <div className="w-[18px] h-[18px] shrink-0 bg-[url('/assets/codia/qU0fn6vNos.png')] bg-cover bg-no-repeat relative overflow-hidden z-[250]" />
                </div>
              </div>
            </div>
            <div className="flex w-[1136px] h-[22px] justify-between items-center relative z-[259]">
              <div className="flex gap-[12px] justify-end items-center grow shrink-0 basis-0 flex-nowrap relative z-[251]">
                <span className="h-[22px] grow shrink-0 basis-auto font-['Vazirmatn'] text-[14px] font-normal leading-[21.875px] text-[#17324d] relative text-right whitespace-nowrap z-[252]">
                  رسید الکترونیکی پرداخت از درگاه معتبر
                </span>
                <div className="flex w-[18px] h-[18px] flex-col justify-center items-center shrink-0 flex-nowrap relative overflow-hidden z-[253]">
                  <div className="w-[18px] h-[18px] shrink-0 bg-[url('/assets/codia/jbmfzbig55.png')] bg-cover bg-no-repeat relative overflow-hidden z-[254]" />
                </div>
              </div>
              <div className="flex gap-[12px] justify-end items-center grow shrink-0 basis-0 flex-nowrap relative z-[255]">
                <span className="h-[22px] grow shrink-0 basis-auto font-['Vazirmatn'] text-[14px] font-normal leading-[21.875px] text-[#17324d] relative text-right whitespace-nowrap z-[256]">
                  نام و جزئیات دقیق پروژه اجتماعی در ماه
                </span>
                <div className="flex w-[18px] h-[18px] flex-col justify-center items-center shrink-0 flex-nowrap relative overflow-hidden z-[257]">
                  <div className="w-[18px] h-[18px] shrink-0 bg-[url('/assets/codia/UGWjztV1WY.png')] bg-cover bg-no-repeat relative overflow-hidden z-[258]" />
                </div>
              </div>
              <div className="flex gap-[12px] justify-end items-center grow shrink-0 basis-0 flex-nowrap relative z-[259]">
                <span className="h-[22px] grow shrink-0 basis-auto font-['Vazirmatn'] text-[14px] font-normal leading-[21.875px] text-[#17324d] relative text-right whitespace-nowrap z-[260]">
                  گواهی هویتی نماینده قانونی معرفی‌شده
                </span>
                <div className="flex w-[18px] h-[18px] flex-col justify-center items-center shrink-0 flex-nowrap relative overflow-hidden z-[261]">
                  <div className="w-[18px] h-[18px] shrink-0 bg-[url('/assets/codia/LGfxighyfs.png')] bg-cover bg-no-repeat relative overflow-hidden z-[262]" />
                </div>
              </div>
            </div>
            <div className="flex w-[1136px] h-[22px] justify-between items-center relative z-[271]">
              <div className="flex gap-[12px] justify-end items-center grow shrink-0 basis-0 flex-nowrap relative z-[263]">
                <span className="h-[22px] grow shrink-0 basis-auto font-['Vazirmatn'] text-[14px] font-normal leading-[21.875px] text-[#17324d] relative text-right whitespace-nowrap z-[264]">
                  نامه رسمی درخواست تاییدیه با امضای مجاز
                </span>
                <div className="flex w-[18px] h-[18px] flex-col justify-center items-center shrink-0 flex-nowrap relative overflow-hidden z-[265]">
                  <div className="w-[18px] h-[18px] shrink-0 bg-[url('/assets/codia/NhOFHDPuQG.png')] bg-cover bg-no-repeat relative overflow-hidden z-[266]" />
                </div>
              </div>
              <div className="flex gap-[12px] justify-end items-center grow shrink-0 basis-0 flex-nowrap relative z-[267]">
                <span className="h-[22px] grow shrink-0 basis-auto font-['Vazirmatn'] text-[14px] font-normal leading-[21.875px] text-[#17324d] relative text-right whitespace-nowrap z-[268]">
                  مستندات احراز صلاحیت و وضعیت مالیاتی
                </span>
                <div className="flex w-[18px] h-[18px] flex-col justify-center items-center shrink-0 flex-nowrap relative overflow-hidden z-[269]">
                  <div className="w-[18px] h-[18px] shrink-0 bg-[url('/assets/codia/cqppEoTK1z.png')] bg-cover bg-no-repeat relative overflow-hidden z-[270]" />
                </div>
              </div>
              <div className="flex gap-[12px] justify-end items-center grow shrink-0 basis-0 flex-nowrap relative z-[271]">
                <span className="h-[22px] grow shrink-0 basis-auto font-['Vazirmatn'] text-[14px] font-normal leading-[21.875px] text-[#17324d] relative text-right whitespace-nowrap z-[272]">
                  تاییدیه اولیه تحقق شاخص‌های فیزیکی پروژه
                </span>
                <div className="flex w-[18px] h-[18px] flex-col justify-center items-center shrink-0 flex-nowrap relative overflow-hidden z-[273]">
                  <div className="w-[18px] h-[18px] shrink-0 bg-[url('/assets/codia/6omcF95pPe.png')] bg-cover bg-no-repeat relative overflow-hidden z-[274]" />
                </div>
              </div>
            </div>
          </div>
        </div>
      </div>
  );
}

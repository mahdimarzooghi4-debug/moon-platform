...
function normalizeRequest(item: unknown): FundPaymentRequest | null {
  ...
  return {
    ...
    note: value.note,
    status: status as FundPaymentStatus,
...
function statusLabel(status: FundPaymentStatus) {
  if (status === "pending_admin") return "در انتظار مدیر ماه";
  if (status === "approved") return "تأیید مدیر ماه";
  if (status === "rejected") return "رد شده";
  return "پرداخت‌شده";
}

function statusClass(status: FundPaymentStatus) {
  if (status === "pending_admin") return "admin-status-review";
  if (status === "rejected") return "admin-access-limited";
...
export default function AdminFunding() {
  ...
  const [view, setView] = useState<"flows" | "fund-requests">(
    initialRequests.some((request) => request.status === "pending_admin") ? "fund-requests" : "flows",
  );
  ...
  const updateStatus = (id: string, status: "approved" | "rejected") => {
    ...
    const updated = requests.map((request) => {
      ...
      if (status === "approved") {
        return { ...request, status: "approved" as const, approvedAt: now, rejectedAt: undefined };
      }
      return { ...request, status: "rejected" as const, rejectedAt: now, approvedAt: undefined };
    });
    setRequests(updated);
      ...
      <main className="admin-users-main" dir="rtl">
        <header className="admin-users-header">
          ...
          <div className="admin-users-actions">
            <button
              className={`admin-users-button ${view === "fund-requests" ? "admin-users-button-primary" : ""}`}
              type="button"
              onClick={() => setView("fund-requests")}
              aria-pressed={view === "fund-requests"}
            >
              درخواست‌های صندوق {pendingCount > 0 ? `(${faNumber(pendingCount)})` : ""}
            </button>
            <button className="admin-users-button admin-list-static-control" type="button" aria-disabled="true">خروجی مالی</button>
          </div>
        </header>
        ...
        <section className="admin-users-toolbar">
          <button className="admin-users-control admin-funding-view-control" type="button" onClick={() => setView("flows")}>جریان‌های تأمین مالی</button>
          <button className="admin-users-control admin-funding-view-control" type="button" onClick={() => setView("fund-requests")}>درخواست‌های پرداخت صندوق</button>
          <div className="admin-users-control admin-list-static-control">{view === "flows" ? "همه وضعیت‌ها" : "همه درخواست‌ها"}</div>
          <div className="admin-users-count">{view === "flows" ? "۲۴ جریان" : `${faNumber(requests.length)} درخواست`}</div>
        </section>
        ...
        <section className="admin-users-table-card admin-list-table-compact">
          {view === "flows" ? (
            ...
            <>
              <h2>درخواست‌های پرداخت از منابع صندوق</h2><p>مدیر صندوق درخواست را ثبت می‌کند و مدیر ماه پیش از ورود آن به مرحله اجرای پرداخت تصمیم می‌گیرد.</p>
              <div className="admin-users-table admin-fund-request-list">
                <div className="admin-users-row admin-users-table-head"><span>پروژه / مجری</span><span>مرحله</span><span>مبلغ</span><span>تاریخ درخواست</span><span>وضعیت</span><span>اقدام مدیر ماه</span></div>
                {sortedRequests.length === 0 ? (
                  <div className="admin-fund-request-empty">درخواست پرداختی از طرف مدیر صندوق ثبت نشده است.</div>
                ) : sortedRequests.map((request) => (
                  <div className="admin-users-row" key={request.id}>
                    <div className="admin-user-cell"><strong>{request.project}</strong><small>{request.executor}</small></div>
                    <span>{request.stage}</span>
                    <span>{formatAmount(request.requestedAmount)}</span>
                    <span>{formatDate(request.createdAt)}</span>
                    <span className={`admin-status-pill ${statusClass(request.status)}`}>{statusLabel(request.status)}</span>
                    <div className="admin-fund-request-actions">
                      {request.status === "pending_admin" ? (
                        <>
                          <button type="button" className="admin-fund-request-approve" onClick={() => updateStatus(request.id, "approved")}>تأیید</button>
                          <button type="button" className="admin-fund-request-reject" onClick={() => updateStatus(request.id, "rejected")}>رد</button>
                        </>
                      ) : request.status === "approved" ? (
                        <span>ارسال‌شده برای اجرا</span>
                      ) : request.status === "paid" ? (
                        <span>{request.receipt ? `رسید ${request.receipt}` : "پرداخت تکمیل شد"}</span>
                      ) : (
                        <span>مختومه</span>
                      )}
                    </div>
        ...
        <aside className="admin-info-note">پرداخت از منابع صندوق فقط پس از تأیید مدیر ماه وارد مرحله اجرای پرداخت می‌شود؛ درخواست‌های آزادسازی خانه خلاق و سهم ۱۰٪ صندوق جریان‌های جداگانه هستند.</aside>
      </main>

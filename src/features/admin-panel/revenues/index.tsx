...
export default function AdminRevenues() {
  const exportRows = () => downloadCsv("admin-revenues", [
    { label: "مبنا / پروژه", value: (item) => item.title },
    { label: "نوع درآمد", value: (item) => item.meta },
    { label: "منبع درآمد", value: (item) => item.source },
    { label: "نرخ", value: (item) => item.rate },
    { label: "مبلغ مبنا", value: (item) => item.base },
    { label: "درآمد ماه", value: (item) => item.income },
  ], revenues);
    ...
    <div className="admin-panel-shell" data-node-id="2279:2">
      <main className="admin-users-main" dir="rtl">
        <header className="admin-users-header">
          ...
          <div className="admin-users-actions"><button className="admin-users-button" type="button" onClick={exportRows}>خروجی درآمدها</button></div>
        </header>
        ...
        <section className="admin-users-toolbar"><div className="admin-users-control admin-list-static-control">جستجو در پروژه، استارتاپ یا شناسه درآمد</div><div className="admin-users-control admin-list-static-control">همه منابع درآمد</div><div className="admin-users-control admin-list-static-control">همه بازه‌ها</div><div className="admin-users-count">۳ جریان</div></section>
        ...
        <section className="admin-users-table-card admin-list-table-compact">
          ...
          <div className="admin-users-table">
            ...
            {revenues.map((item, index) => <div className="admin-users-row" key={item.title}><div className="admin-user-cell"><strong>{item.title}</strong><small>{item.meta}</small></div><span>{item.source}</span><span className="admin-status-pill admin-status-active">{item.rate}</span><span>{item.base}</span><span className="admin-access-pill admin-access-full">{item.income}</span><Link className="admin-user-action" to={`/panel/admin/revenues/${index + 1}`}>مشاهده</Link></div>)}
            <div className="admin-pagination"><span>نمایش ۱ تا ۳ از ۳ جریان</span><div className="admin-pagination-controls"><button className="admin-page-control" type="button" disabled>‹</button><span className="admin-page-number">۱</span><button className="admin-page-control" type="button" disabled>›</button></div></div>
          </div>
        </section>

        <section className="admin-revenue-schedule"><h2>نرخ کارمزد تأمین مالی استارتاپ‌ها</h2><p>نرخ بر اساس مبلغ کل تأمین مالی پروژه محاسبه می‌شود.</p><div className="admin-revenue-tiers">{tiers.map((tier) => <div className="admin-revenue-tier" key={tier}>{tier}</div>)}</div></section>
      </main>

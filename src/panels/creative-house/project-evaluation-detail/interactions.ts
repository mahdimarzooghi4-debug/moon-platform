const DETAIL_ROOT_SELECTOR =
  '.creative-house-dashboard[data-name="ayeneh-project-evaluation-detail"]';
...
const PROJECT_LIST_ROUTE = "/panel/creative-house/project-evaluations";
const PROJECT_DECISION_STORAGE_KEY =
  "mah.creativeHouse.projectEvaluationDecision.v1";

const DECISION_BY_NAME: Record<string, string> = {
  "decision-option-1": "approve",
  "decision-option-2": "request-revision",
...
function applyProjectDetailInteractions(root: ParentNode = document) {
  root.querySelectorAll<HTMLElement>(DETAIL_ROOT_SELECTOR).forEach((screen) => {
    ...
    group?.setAttribute("aria-label", "تصمیم نهایی پروژه");
    ...
    save?.setAttribute("href", PROJECT_LIST_ROUTE);

    const back = screen.querySelector<HTMLAnchorElement>(BACK_TO_LIST_SELECTOR);
    back?.setAttribute("href", PROJECT_LIST_ROUTE);
  });
}
...
document.addEventListener("click", (event) => {
  ...
  const back = target.closest<HTMLAnchorElement>(
    `${DETAIL_ROOT_SELECTOR} ${BACK_TO_LIST_SELECTOR}`,
  );
  if (back) {
    event.preventDefault();
    navigateWithinApp(PROJECT_LIST_ROUTE);
    return;
  }
  ...
  if (!selected) {
    event.preventDefault();
    window.alert("لطفاً یک تصمیم نهایی برای پروژه انتخاب کنید.");
    return;
  }
  ...
  const approvedFunding =
    screen
      .querySelector<HTMLElement>('[data-name="approved-funding-amount"]')
      ?.textContent?.replace(/\s+/g, " ")
      .trim() ?? "";

  localStorage.setItem(
    PROJECT_DECISION_STORAGE_KEY,
    JSON.stringify({
      decision,
...
const start = () => {
  applyProjectDetailInteractions();
  const observer = new MutationObserver(() => applyProjectDetailInteractions());

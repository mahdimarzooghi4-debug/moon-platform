...
function FlowCard({ label, icon }: { label: string; icon: string }) {
  return (
    <div className="flex h-[86px] w-[160px] shrink-0 flex-col items-center justify-center gap-[10px] rounded-[12px] border border-[#e4ebf1] bg-white p-[16px] shadow-[0_4px_8px_rgba(23,46,71,0.08)]">
      <img src={icon} alt="" className="h-[24px] w-[24px] shrink-0" />
      <p className="w-full whitespace-nowrap text-center font-['Vazirmatn'] text-[13px] font-bold leading-[20px] text-[#17324d]" dir="auto">
        {label}
      </p>
    </div>
  );
    ...
    <section className="flex flex-col items-start gap-[40px] px-[120px] py-[80px]" data-node-id="745:4432">
      ...
      <div className="flex w-full items-start justify-center gap-[16px]">
        {steps.map((step, index) => (
          <div key={step.label} className={`flex shrink-0 items-center ${index === 0 ? "" : "gap-[12px]"}`}>

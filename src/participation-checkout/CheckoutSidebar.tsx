...
function Row({ label, value, valueClass = "" }: { label: string; value: string; valueClass?: string }) {
  return (
    <div className="flex w-full items-start justify-between text-[13px] leading-5" dir="ltr">
      ...
      <span className="text-right text-[#60758a]" dir="rtl">{label}</span>
    </div>
  );
}
...
function TrustItem({ icon, title, description }: { icon: string; title: string; description: string }) {
  return (
    <div className="flex w-full items-center justify-end gap-3" dir="ltr">
      <div className="flex min-w-0 flex-1 flex-col items-start gap-0.5 text-right" dir="rtl">
        <strong className="w-full text-right text-[13px] leading-5 text-[#17324d]">{title}</strong>
        <span className="w-full text-right text-[11px] leading-[17px] text-[#60758a]">{description}</span>
      </div>
...
export default function CheckoutSidebar({ amount, onProject, onCompany }: { amount: number; onProject: () => void; onCompany: () => void }) {
  return (
    <aside className="flex w-[370px] shrink-0 flex-col gap-6 text-right" dir="rtl">
      <section className="flex w-full flex-col gap-5 rounded-3xl border border-[#e4ebf1] bg-white p-6 shadow-[0_4px_16px_rgba(23,46,71,0.08)]" dir="rtl">
        ...
        <div className="flex w-full flex-col items-start gap-2 text-right">
          <span className="w-full text-right text-[12px] text-[#2094e3]">حوزه: اشتغال و معیشت | استان: گلستان</span>
          <strong className="w-full text-right text-[18px] leading-7">اشتغال زنان روستایی</strong>
          <span className="w-full text-right text-[13px] text-[#60758a]">استارتاپ مجری: راهکار سبز</span>
        </div>
      ...
      <section className="flex w-full flex-col gap-4 rounded-[20px] border border-[#e4ebf1] bg-white p-5" dir="rtl">
      ...
      <section className="flex w-full flex-col items-start gap-3 rounded-2xl bg-[#eaf5fd] p-5 text-right" dir="rtl">
        <strong className="w-full text-right text-[13px] text-[#2094e3]">ثبت مشارکت به صورت حقیقی</strong>
        <p className="m-0 w-full text-right text-[12px] leading-5 text-[#60758a]">برای مشارکت فردی نیازی به حساب کاربری سازمانی نیست و گواهی مالیاتی موضوع ماده ۱۷۲ برای اشخاص حقیقی صادر نمی‌شود.</p>
        <button type="button" onClick={onCompany} className="cursor-pointer border-0 bg-transparent p-0 text-right text-[12px] font-bold text-[#2094e3] underline">مشارکت از طرف شرکت و دریافت گواهی مالیاتی ←</button>
      </section>
    </aside>
  );
}
